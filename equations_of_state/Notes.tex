...
\begin{equation}
  Pv-RT = 0
\end{equation}
The IGL ignores both the volume of the molecules in the gas, and all electrostatic interactions between them (hard-sphere model).
While this makes the IGL simple to use, these assumptions do not allow the IGL to describe realistic gases and pressures; IGL describes gas phase behavior in the limit of low pressure.
...
Then, by definition the compressibility factor for an ideal gas is Z=1.
As the behavior of the gas deviates from the IGL, the compressibility factor can be used a correction.
The model for how the compressibility changes with pressure (or alternatively with molar volume) is called the
Virial equation of state.  In particular, the Virial equation of state is the power-series of either pressure $P$ or
...
The Virial coefficients $\alpha_{i}$ and $\alpha^{\prime}_{i}$ (which are related with one another and are strong functions of temperature)
describe the strength of intermolecular interactions (interactions between molecules) in the fluid.
These coefficients can be estimated from data for each material of interest, or they can be derived
...
\begin{mdframed}[backgroundcolor=lgray]
	\noindent\textbf{Example}: Estimate the second and Virial coefficient ($\alpha_{1}$) for CH$_\mathrm{4}$ from compressibility data.

	\noindent\textbf{Solution.}\\

	To estimate the second Virial coefficient 
