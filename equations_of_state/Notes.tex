...
An equation of state is a function $\mathcal{F}(\cdot)$ that relates the state variables, temperature $T$, pressure $P$, volume $V$ and composition $n$
of a system such that:
\begin{equation}
\mathcal{F}\left(P,T,V,n\right) = 0
\end{equation}
Thus, equations of state are mathematical models of the physical behavior of pure substances (that can also be adapted to mixtures with a few modifications).
In theory, we should be able to formulate an equation of state that describes all phases of a pure substance (or a mixture) using our understanding of the
chemical and mechanical properties of the substance.
However, in practice this is not often the case; most equations of state describe only single phase behavior, and are sometimes not physically motivated.
